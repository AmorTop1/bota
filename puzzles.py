...
def puzzles():
    mess = ""
    with open("puzzles_link.json", "r", encoding="utf-8") as f:
        data = json.load(f)
...
@bot.message_handler(func=lambda x: x.text.startswith('Головоломки'))
def showArcades(message):
    bot.send_message(message.chat.id, puzzles())

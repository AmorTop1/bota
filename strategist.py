...
def strategist():
    mess = ""
    with open("strategist_link.json", "r", encoding="utf-8") as f:
        data = json.load(f)
...
@bot.message_handler(func=lambda x: x.text.startswith('Стратегіїчні'))
def showArcades(message):
    bot.send_message(message.chat.id, strategist())

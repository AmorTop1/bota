...
def shooter():
    mess = ""
    with open("shooter_link.json", "r", encoding="utf-8") as f:
        data = json.load(f)
...
@bot.message_handler(func=lambda x: x.text.startswith('Шутери'))
def showArcades(message):
    bot.send_message(message.chat.id, shooter())
